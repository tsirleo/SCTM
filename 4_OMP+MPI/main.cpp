...
void save_statistics(const Grid& g, double& time, double& max_inaccuracy, double& first_step_inaccuracy, double& last_step_inaccuracy, int& proc_num, int& threads_num) {
    std::string filename = "results/statistics/" + std::to_string(g.N) + "_" + std::to_string(proc_num) + "_" + std::to_string(threads_num) + "_" + g.L_type + "_" + "statistics.txt";
    std::ofstream file(filename);
    if (!file.is_open()) {
        std::cerr << "Failed to open file '" << filename << "' for writing." << std::endl;
        return;
...
//    block.print_block_info();
    ...
    if (rank == 0) {
        std::cout << "Result: \n\t" << "Time = " << time << "\n\t" << "Max inaccuracy = " << max_inaccuracy << "\n\t" << "First step inaccuracy = " << first_step_inaccuracy << "\n\t" << "Last step inaccuracy = " << last_step_inaccuracy << std::endl;
        save_statistics(grid, time, max_inaccuracy, first_step_inaccuracy, last_step_inaccuracy, proc_num, threads_num);
    }

//    dump_block_to_CSV(grid, block, result_vec, "results/grid/" + std::to_string(N) + "_" + std::to_string(proc_num) + "_" + std::to_string(threads_num) + "_" + grid.L_type + "_" + NUMERICAL + std::to_string(block.dim0_rank) + std::to_string(block.dim1_rank) + std::to_string(block.dim2_rank) + ".csv", NUMERICAL);
//    dump_block_to_CSV(grid, block, result_vec, "results/grid/" + std::to_string(N) + "_" + std::to_string(proc_num) + "_" + std::to_string(threads_num) + "_" + grid.L_type + "_" + ANALITICAL + std::to_string(block.dim0_rank) + std::to_string(block.dim1_rank) + std::to_string(block.dim2_rank) +  ".csv", ANALITICAL);
//    dump_block_to_CSV(grid, block, result_vec, "results/grid/" + std::to_string(N) + "_" + std::to_string(proc_num) + "_" + std::to_string(threads_num) + "_" + grid.L_type + "_" + INACCURACY + std::to_string(block.dim0_rank) + std::to_string(block.dim1_rank) + std::to_string(block.dim2_rank) +  ".csv", INACCURACY);
