...
void save_statistics(const Grid& g, double& time, double& max_inaccuracy, double& first_step_inaccuracy, double& last_step_inaccuracy) {
    std::string filename = "results/statistics/" + std::to_string(g.N) + "_" + g.L_type + "_" + "statistics.txt";
    std::ofstream file(filename);
    if (!file.is_open()) {
        std::cerr << "Failed to open file '" << filename << "' for writing." << std::endl;
        return;
...
int main(int argc, char* argv[]) {
    ...
    solve_equation(grid, time, max_inaccuracy, first_step_inaccuracy, last_step_inaccuracy, result_vec);

    std::cout << "Result: \n\t" << "Time = " << time << "\n\t" << "Max inaccuracy = " << max_inaccuracy << "\n\t" << "First step inaccuracy = " << first_step_inaccuracy << "\n\t" << "Last step inaccuracy = " << last_step_inaccuracy << std::endl;
    save_statistics(grid, time, max_inaccuracy, first_step_inaccuracy, last_step_inaccuracy);


    dump_grid_to_CSV(grid, result_vec, "results/grid/" + std::to_string(N) + "_" + grid.L_type + "_" + NUMERICAL + ".csv", NUMERICAL);
    dump_grid_to_CSV(grid, result_vec, "results/grid/" + std::to_string(N) + "_" + grid.L_type + "_" + ANALITICAL + ".csv", ANALITICAL);
    dump_grid_to_CSV(grid, result_vec, "results/grid/" + std::to_string(N) + "_" + grid.L_type + "_" + INACCURACY + ".csv", INACCURACY);
